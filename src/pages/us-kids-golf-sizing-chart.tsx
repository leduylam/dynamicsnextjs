...
export default function UsKidGolfSizingChart() {
  return (
    <>
      <PageHeader pageHeader="Us kid's Golf Sizing Chart" />
                ...
                >
                  ...
                  >
                    {item.description.map((title, index) => (
                      <div key={index}>
                        <h2 className='text-lg text-heading font-bold text-center'>{title.appTitle}</h2>
                        {
                              ...
                              <table className='w-full'>
                                ...
                                <tbody>
                                  {table.tbody.map((a, index) => (
                                    <tr key={index}>
                                      {a.tr.map((b, index) => (
                                        <td className={`${typeof b === 'object' ? b.className : ''}`}
                                          align='center'
                                          key={index}
                                          dangerouslySetInnerHTML={{ __html: typeof b === 'object' ? b.content : b }}
                                        />
                                      ))}
                                    </tr>
                                  ))}
