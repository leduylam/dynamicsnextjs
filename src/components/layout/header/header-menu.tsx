...
const HeaderMenu: React.FC<MenuProps> = ({ data, className }) => {
  const hasSubCategory = (categories: any[]): boolean => {
    return categories.some((category) =>
      ...
      || (Array.isArray(category.subCategory) && hasSubCategory(category.subCategory)) // Kiểm tra sâu hơn nếu có subCategory lồng nhau
    );
  };
  ...
  return (
    <nav className={classNames(`headerMenu flex w-full relative`, className)}>
      {data?.map((item: any) => {
        const showDropdown = Array.isArray(item.subCategory) && item.subCategory.length > 0;
        const href = isFullLink(item.slug) ? item.slug : `/search?category=${item.slug}`;
        return (
          <div
          ...
          >
            <Link
              href={href}
              {...(isFullLink(href) ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
